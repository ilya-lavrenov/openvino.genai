...
inline ov::genai::ModelDesc extract_draft_model_from_config(ov::AnyMap& config) {
...
inline bool extract_prompt_lookup_from_config(ov::AnyMap& config) {
    bool res = false;
...
ContinuousBatchingPipeline::ContinuousBatchingPipeline(const std::filesystem::path& models_path,
    ...
    if (is_prompt_lookup_enabled) {
        OPENVINO_ASSERT(draft_model_desr.model == nullptr,
                        "Speculative decoding and prompt lookup decoding are mutually excluded");
