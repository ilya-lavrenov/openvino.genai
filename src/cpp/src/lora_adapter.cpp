...
BufferPtr read_file_helper(const std::filesystem::path& filename) {
    ...
    // TODO: Use mmapped AlignedBuffer as ov::Core::read_model can do, necessary functionality is not available in
    // public OV API. LoRA files do not usually have huge size in comparison to the base models, but it can vary
    // depending on adapter, and using mmap will help to optimize memory consumption and could be critical when the
    // application at the edge of available memory that is not really uncommon for applications dealing with LLMs.
...
// The data is read to a solid memory block and the function returns a map of OV Constants allocated on top of that
// block. The key in the map is a tensor name and the Constant uses a region of memory from the memory block. Each
// Constant holds a shared pointer to the block in the runtime info. The memory block will be deallocated when the last
// Constant is destroyed.
ConstantMap read_safetensors(const std::filesystem::path& filename) {
    ...
    OPENVINO_ASSERT(safetensors_file_init(&(*buffer)[0], buffer->size(), &safe_tensors_file) == nullptr,
                    "Cannot parse ",
                    filename,
                    " as a Safetensors file format. Safetensors file format is supported only");
    ...
    for (int i = 0; i < safe_tensors_file.num_tensors; i++) {
        ...
        OPENVINO_ASSERT(ov::shape_size(shape) <= tensor.end_offset_bytes - tensor.begin_offset_bytes,
                        ...
                        filename,
                        "\" doesn't match the expected tensor size ",
        ...
        constant->get_rt_info()["__safetensors_buffer_holder"] =
            buffer;  // to automatically deallocate underlying memory buffer when last constant that holds it is
                     // destroyed
...
LoRAPartsParser default_lora_patterns() {
    return LoRAPartsParser(RegexParser(R"((.*)\.alpha)", 1),
...
struct LoRAWeightGetterDefault {
    ...
    LoRAWeightGetterDefault(const LoRATensors* lora_tensors, const std::string& prefix)
    ...
    std::optional<LoRANode> operator()(const std::string& name) const {
        ...
        auto it = std::find_if(
            lora_tensors->begin(),
            lora_tensors->end(),
            [this, variants](const LoRATensors::value_type& pair) {
...
#endif
        model->add_sinks(
            {std::make_shared<v6::Assign>(read_value,
                                          variable)});  // FIXME: Required? -- Yes, create ticket against CPU
        return read_value;
    }
};
...
NodePtr decompression_convert(NodePtr node) {
    ...
    OPENVINO_ASSERT(std::dynamic_pointer_cast<v0::Constant>(node),
                    "Not supported decompression pattern at the weight input (presumably low-bit compression). Use "
                    "f32/f16/bf16 weights only.");
    return convert;
}
...
public:
    ...
    bool apply(NodePtr node, const LoRANode& lora_weight) override {
        ...
        // Newly created constants in the next line are not mmaped unlike original weights, so it will inflate required
...
Adapter::Adapter(const std::filesystem::path& path) : m_pimpl(std::make_shared<Adapter::Impl>(path)) {}
...
struct AdapterControllerImpl {
          ...
          lora_state_evaluators("CPU")  // FIXME: Try to run on the same device that is used for model inference
    {
        ...
        for (auto const& adapter : current_config.get_adapters()) {
            ...
            params_getter.weight_getter.push_back(
                LoRAWeightGetterDefault(&adapter_impl->tensors, config.get_tensor_name_prefix().value_or("")));
    ...
    void set_new_adapter_tensors(ov::InferRequest& infer_request, bool alpha_only = false) {
        ...
        for (const auto& adapter : adapters) {
            weight_getters.emplace_back(LoRAWeightGetterDefault(&get_adapter_impl(adapter)->tensors,
