...
public:
    ...
    StatefulLLMPipeline(const std::filesystem::path& models_path,
        ...
        : StatefulLLMPipeline{ov::genai::utils::read_model_with_config(models_path, plugin_config),
                              tokenizer,
                              device,
                              plugin_config,
    ...
    StatefulLLMPipeline(const std::shared_ptr<ov::Model>& model,
        ...
        if (auto filtered_plugin_config =
                extract_adapters_from_properties(plugin_config, &m_generation_config.adapters)) {
            m_generation_config.adapters->set_tensor_name_prefix("base_model.model.model.");
            m_adapter_controller = AdapterController(model,
                                                     *m_generation_config.adapters,
                                                     device);  // TODO: Make the prefix name configurable
    ...
    DecodedResults generate(StringInputs inputs,
        ...
        if (is_chat_conversation)
            OPENVINO_ASSERT(m_chat_input_type != ov::genai::utils::GenerationChatInputsType::ENCODED_INPUTS,
                            "Chat doesn't support switching between input types. Please, continue using EncodedInputs "
                            "or restart the chat.");
        ...
        if (auto input_vector = std::get_if<std::vector<std::string>>(&inputs)) {
            ...
            if (is_chat_conversation) {
                ...
                // So actual pipeline calculates input_ids for whole chat history + for whole chat history without the
                // new prompt and takes only the difference between them. The chat history cannot be saved as already
                // encoded tokens because generate call doesn't return <eos> token, but KV cache contains it. So we have
                // to add it manually or get it by tokenization all chat history.

                m_history.push_back({{"role", "user"}, {"content", prompt}});
                constexpr bool add_generation_prompt = true;
                auto new_templated_chat_history = m_tokenizer.apply_chat_template(m_history, add_generation_prompt);
                // Do not add special tokens in chat scenario to be aligned with HF.
                auto new_chat_tokens =
                    m_tokenizer.encode(new_templated_chat_history, ov::genai::add_special_tokens(false));
                auto prev_chat_tokens =
                ...
                // if we met sequence with such combination of symbols, we cannot correctly subtract the new history
                // from the old history so let's check it out, find the trusted part and use it in on the next step
                size_t last_same_hist_token = 0;
                if (!m_tokenized_chat_history.empty()) {
                    std::set<int64_t> stop_tokens = config.stop_token_ids;
                    last_same_hist_token = ov::genai::utils::get_first_history_difference(prev_chat_tokens.input_ids,
                ...
                if (m_tokenized_chat_history.empty()) {
                    ...
                    ov::Tensor new_tensor =
                        ov::Tensor(new_chat_tokens.input_ids.get_element_type(),
                ...
                std::copy_n(new_chat_tokens.input_ids.data<int64_t>(),
                            new_chat_tokens.input_ids.get_size(),
                            std::back_inserter(m_tokenized_chat_history));

                // TODO: Forbid LoRA config change if we are in the chat mode, because it requires regenerating the
                // history with LoRA applied
            } else {
    ...
    EncodedResults generate(const EncodedInputs& inputs,
        ...
        if (is_chat_conversation)
            // if chat was run in StringInputs mode, but it was called EncodedInputs generate, last m_history entry will
            // be with assistant role
            OPENVINO_ASSERT(m_chat_input_type == ov::genai::utils::GenerationChatInputsType::ENCODED_INPUTS ||
                                m_history.back()["role"] == "user",
                            "Chat doesn't support switching between input types. Please, continue using StringInputs "
                            "or restart the chat.");
        ...
        ov::genai::utils::trim_kv_cache(m_model_runner,
                                        m_to_remove_from_hist,
                                        m_kv_cache_seq_length_axis,
        ...
        if (config.is_beam_search() && is_chat_conversation) {
            ...
            for (size_t request_id = 0; request_id < batch_size; request_id++) {
                ...
                if (is_chat_conversation) {
                    ov::Tensor tokenized_chat_history = ov::Tensor(ov::element::i64,
                                                                   {1, m_tokenized_chat_history.size()},
                                                                   m_tokenized_chat_history.data());
            ...
            std::tie(result, m_selected_beam) = ov::genai::get_lm_encoded_results(m_model_runner,
                                                                                  input_ids,
                                                                                  concatenated_attention_mask,
                                                                                  streamer_ptr,
                                                                                  sampler,
...
public:
    ...
    ContinuousBatchingAdapter(const ov::InferRequest& request,
                              ...
                              OptionalGenerationConfig generation_config)
        : LLMPipelineImplBase{dont_construct()},
          m_impl{{}, {}, {}} {}
    ...
    ContinuousBatchingAdapter(const std::filesystem::path& models_path,
                              ...
                              const ov::AnyMap& plugin_config)
        : LLMPipelineImplBase{tokenizer},
          m_impl{models_path.string(), tokenizer, scheduler_config, device, plugin_config} {
    ...
    ContinuousBatchingAdapter(const std::string& model_str,
                              ...
                              const ov::genai::GenerationConfig& generation_config)
        : LLMPipelineImplBase{tokenizer},
          m_impl{model_str, weights_tensor, tokenizer, scheduler_config, device, plugin_config, generation_config} {}

    ContinuousBatchingAdapter(const std::filesystem::path& models_path,
                              const SchedulerConfig& scheduler_config,
                              const std::string& device,
                              const ov::AnyMap& plugin_config)
        : LLMPipelineImplBase{Tokenizer(models_path.string())},
          m_impl{models_path.string(), m_tokenizer, scheduler_config, device, plugin_config} {
        m_generation_config = m_impl.get_config();
    }
...
ov::genai::LLMPipeline::LLMPipeline(const std::string& model_str,
    ...
    if (plugin_config.find(ov::genai::scheduler_config.name()) != plugin_config.end()) {
        ...
        m_pimpl = std::make_unique<StaticLLMPipeline>(utils::singleton_core().read_model(model_str, weights_tensor),
                                                      model_descr,
                                                      tokenizer,
                                                      device,
                                                      properties,
