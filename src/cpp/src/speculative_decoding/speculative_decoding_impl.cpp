...
ContinuousBatchingPipeline::SpeculativeDecodingImpl::SpeculativeDecodingImpl(
    ...
    ov::genai::SchedulerConfig main_scheduler_config_updated = main_scheduler_config,
                               draft_scheduler_config =
                                   is_scheduler_undefined ? main_scheduler_config : draft_model_desc.scheduler_config;
    ...
    ov::AnyMap draft_properties =
        draft_model_desc.properties == ov::AnyMap{} ? compile_properties : draft_model_desc.properties;
    ...
    m_main_pipeline = std::make_shared<ContinuousBatchingForSpeculativeDecodingImpl>(core,
                                                                                     ...
                                                                                     true);
    m_draft_pipeline =
        std::make_shared<ContinuousBatchingForSpeculativeDecodingImpl>(core,
                                                                       draft_model,
...
std::vector<EncodedGenerationResult> ContinuousBatchingPipeline::SpeculativeDecodingImpl::generate(
    ...
    for (size_t request_id = 0; request_id < input_ids.size(); ++request_id) {
        ...
        main_generations.push_back(
            m_main_pipeline->add_request(request_id, input_ids[request_id], sampling_params[request_id]));
        ...
        std::lock_guard<std::mutex> lock(m_draft_generations_mutex);
        m_draft_generations.insert(
            {request_id, m_draft_pipeline->add_request(request_id, input_ids[request_id], draft_sampling_params)});
    }
