...
int match_stop_string(Tokenizer& tokenizer,
    ...
    */
    ...
    for (auto stop_string : stop_strings) {
        ...
        while (generated_tokens_rit != generated_tokens.rend()) {
            ...
            if (clean_text == "" ||
                (clean_text.size() >= 3 && (clean_text.compare(clean_text.size() - 3, 3, "�") == 0))) {
    ...
    for (size_t group_id = 0; group_id < m_groups.size(); ++group_id) {
        ...
        for (size_t cand_idx = 0; cand_idx < candidates.size(); ++cand_idx) {
            ...
            if (!m_parameters.stop_strings.empty()) {
                ...
                int num_last_matched_tokens =
                    match_stop_string(m_tokenizer, token_ids, m_sequence_group->get_sampling_parameters().stop_strings);
...
std::vector<int64_t> Sampler::_try_finish_generation(SequenceGroup::Ptr& sequence_group) {
    ...
    for (auto& running_sequence : sequence_group->get_running_sequences()) {
        ...
        if (!sampling_params.stop_strings.empty()) {
            int num_matched_last_tokens =
                match_stop_string(m_tokenizer, running_sequence->get_generated_ids(), sampling_params.stop_strings);
...
bool Sampler::validate_candidate(Sequence::Ptr running_sequence,
    ...
    if (do_sample && running_sequence->get_generated_len() !=
...
SamplerOutput Sampler::sample(std::vector<SequenceGroup::Ptr>& sequence_groups,
    ...
    for (size_t sequence_group_id = 0, currently_processed_tokens = 0; sequence_group_id < sequence_groups.size();
        ...
        size_t actual_seq_len =
            sequence_group->get_num_scheduled_tokens();  // points to a token which needs to be sampled
        ...
        if (sequence_group->requires_sampling()) {
            ...
            if (sampling_params.is_greedy_decoding() || sampling_params.is_multinomial()) {
                ...
                for (size_t running_sequence_id = 0; running_sequence_id < num_running_sequences;
                     ++running_sequence_id) {
                    ...
                    for (size_t i = 0; i <= num_tokens_to_process; ++i) {
                        ...
                        if (sampling_params.is_greedy_decoding()) {
                            ...
                            if (is_generate_n_tokens) {
                                ...
                                sampler_output.m_forked_sequences.insert(
